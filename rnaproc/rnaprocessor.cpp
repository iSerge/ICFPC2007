...
void setPixel(QImage *img, int x, int y, QColor c){
...
void RNAprocessor::compose(){
    ...
    for(int y = 0; y < 600; y++){
        for(int x = 0; x < 600; x++){
            ...
            image1->setPixel(x, y, qRgba(qRed(c0)    + qRed(c1)  *(255 - qAlpha(c0))/255,
...
void RNAprocessor::clip(){
    ...
    for(int y = 0; y < 600; y++){
        for(int x = 0; x < 600; x++){
            QRgb c0 = img0->pixel(x, y);
            QRgb c1 = image1->pixel(x, y);
            image1->setPixel(x, y, qRgba(qRed(c1)  *qAlpha(c0)/255,
...
QColor RNAprocessor::currentPixel(){
